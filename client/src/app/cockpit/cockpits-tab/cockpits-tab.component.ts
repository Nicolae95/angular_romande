...
export class CockpitsTabComponent implements OnInit {
  ...
  cockpitB() {
    this.customObservable.changecockpitB(true);
  }
  ...
  sendCockpitOnMail(id: number) {
    this.cockpitService.sendNow(id).subscribe(
      (data) => { this.toastr.success('L\'cockpit a été envoyée.'); },
      (er) => {},
    );
  }
